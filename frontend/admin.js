...
(function () {
  ...
  let currentFiles = [];
  let adminToken = sessionStorage.getItem('study_admin_token') || '';

  // initialize token input
  if (adminToken) tokenInput.value = '••••••••';

  function requireToken() {
    if (!adminToken) {
      const t = prompt('Admin token required — enter token (will be stored for session)');
      if (t) { adminToken = t.trim(); sessionStorage.setItem('study_admin_token', adminToken); tokenInput.value = '••••••••'; showToast('Token set'); }
    }
  }

  function clearToken() { adminToken = ''; sessionStorage.removeItem('study_admin_token'); tokenInput.value = ''; showToast('Token cleared'); }
  ...
  async function uploadHandler(e) {
    ...
    const res = await fetch(API_BASE + '/upload', { method: 'POST', body: form, headers: adminToken ? { 'x-admin-token': adminToken } : {} });
    const data = await res.json();
  ...
  closeViewer.addEventListener('click', () => { viewerModal.classList.add('hidden'); viewerBody.innerHTML = ''; });
  ...
  applyTokenBtn.addEventListener('click', () => { const t = tokenInput.value.trim(); if (!t) return showToast('Enter token'); adminToken = t; sessionStorage.setItem('study_admin_token', adminToken); tokenInput.value = '••••••••'; showToast('Token set'); });
  ...
  searchInput.addEventListener('input', () => { clearTimeout(searchTimer); searchTimer = setTimeout(() => { const q = searchInput.value.trim().toLowerCase(); if (!q) return renderFiles(); const filtered = currentFiles.filter(f => (f.name || '').toLowerCase().includes(q) || (f.folder || '').toLowerCase().includes(q)); currentFiles = filtered; renderFiles(); }, 300); });

  // initial load
  (async () => { requireToken(); await listFiles(); })();
})();
